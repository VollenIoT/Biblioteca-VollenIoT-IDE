...
const char* VollenIoT::_deviceLabel(int idx) const {
    return (_devices[idx].name[0] != '\0') ? _devices[idx].name : _devices[idx].id;
}
...
bool VollenIoT::_addDeviceInternal(const char* id, const char* name,
    ...
    strncpy(_devices[_deviceCount].comandoOff, "0", sizeof(_devices[_deviceCount].comandoOff) - 1);
    strncpy(_devices[_deviceCount].comandoOn, "1", sizeof(_devices[_deviceCount].comandoOn) - 1);
    if (name) {
...
bool VollenIoT::setDeviceCommand(const char* id, const char* comandoOff, const char* comandoOn) {
    int idx = _findDevice(id);
    if (idx < 0) {
        Serial.printf("[VollenIoT] Dispositivo nao encontrado: %s\n", id);
        return false;
    }
    strncpy(_devices[idx].comandoOff, comandoOff, sizeof(_devices[idx].comandoOff) - 1);
    strncpy(_devices[idx].comandoOn, comandoOn, sizeof(_devices[idx].comandoOn) - 1);
    Serial.printf("%s Comandos: %s (OFF) / %s (ON)\n", _deviceLabel(idx), _devices[idx].comandoOff, _devices[idx].comandoOn);
    return true;
}
...
void VollenIoT::setDeveloperMode(bool enable) {
    ...
    if (enable) {
        Serial.println("[VollenIoT] Modo Desenvolvedor ATIVADO. Multi-device ID permitido.");
    }
}
...
void VollenIoT::setDeviceState(const char* id, bool newState) {
    ...
    if (mudou && (_devices[idx].pendingMinutes > 0 || _devices[idx].pendingSeconds > 0)) {
        if (newState != _devices[idx].pendingAction) {
            unsigned long totalMs = ((unsigned long)_devices[idx].pendingMinutes * 60000UL) + ((unsigned long)_devices[idx].pendingSeconds * 1000UL);
            ...
            Serial.printf("%s Timer iniciado: %d min e %d seg para %s\n",
                          _deviceLabel(idx), _devices[idx].pendingMinutes, _devices[idx].pendingSeconds,
...
void VollenIoT::connect() {
    if (WiFi.status() != WL_CONNECTED) {
        Serial.println("[VollenIoT] Wi-Fi nao conectado. Chame WiFi.begin() no seu sketch.");
        return;
    }
    _connectMQTT();
}
...
bool VollenIoT::isMQTTConnected() {
...
// -----------------------------------------------------------------------------
bool VollenIoT::_processMessage(const char* topic, const uint8_t* payload, size_t length) {
    char message[256];
    ...
    if (slash) {
        ...
        if (idLen >= sizeof(deviceId)) idLen = sizeof(deviceId) - 1;
        ...
        if (strcmp(slash + 1, "status") == 0) {
            ...
            if (idx >= 0) {
                ...
                if (sscanf(message, "online:%lu", &otherBootId) == 1 && otherBootId != _bootId) {
                    if (!_developerMode) {
                        Serial.printf("\n[VollenIoT] ERRO: ID duplicado detectado para %s! Desconectando do MQTT...\n\n", deviceId);
                        _mqttClient.disconnect();
                        _duplicateDetected = true;
                    } else {
                        if (!_duplicateDetected) {
                            _duplicateDetected = true;
                            Serial.printf("\n*** AVISO: ID duplicado detectado (Modo Desenvolvedor Ativo)! ***\n");
                            Serial.printf("*** Dispositivo %s ja esta online em outro ESP ***\n", deviceId);
                            Serial.printf("*** Ambos responderao aos mesmos comandos. ***\n\n");
                        }
                    }
                }
            }
    ...
    if (message[0] == '{') {
        ...
        unsigned long totalMs = ((unsigned long)minutes * 60000UL) + ((unsigned long)seconds * 1000UL);

        if (_devices[idx].state != alvo) {
            _devices[idx].timerEnd = millis() + totalMs;
            _devices[idx].timerAction = alvo;
            Serial.printf("%s Temporizador ativo: %d min e %d seg para %s\n",
                          _deviceLabel(idx), minutes, seconds, alvo ? "Ligar" : "Desligar");
        } else {
            _devices[idx].timerEnd = 0;
            Serial.printf("%s Temporizador pendente: %d min e %d seg para %s (iniciara assim que o estado mudar para %s)\n",
                          _deviceLabel(idx), minutes, seconds, alvo ? "Ligar" : "Desligar",
                          alvo ? "DESLIGADO" : "LIGADO");
        }
        return true;
    }
    ...
    // Verifica se o comando recebido corresponde ao par configurado para este dispositivo
    bool newState = false;
    bool matched = false;
...
// -----------------------------------------------------------------------------
void VollenIoT::_mqttCallback(char* topic, uint8_t* payload, unsigned int length) {
    if (_instance) {

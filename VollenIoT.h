...
public:
    ...
    bool setDeviceCommand(const char* id, const char* comandoOff, const char* comandoOn);
...
private:
    ...
    bool _addDeviceInternal(const char* id, const char* name, uint8_t pin, bool state);
    void _setupPins();
    void _connectMQTT();
    bool _processMessage(const char* topic, const uint8_t* payload, size_t length);
